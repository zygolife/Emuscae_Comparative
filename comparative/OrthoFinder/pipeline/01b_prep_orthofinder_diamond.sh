...
opt="-p short -C ryzen -N 1 -n 16 --mem 8gb" # could change to "-C xeon" and will run on the xeon nodes; # could change this to empty and will run on any node
...
grep ^diamond $LOG | grep -v 'commands that must be run' | perl -p -e 's/-p 1/-p 16/g'> $JOBS
...
#	echo "$START,$END for $n"
	...
	sbatch $opt --out logs/diamond.$n.log -J Dmd$n --wrap "module load orthofinder/2.5.2; $run"
done
