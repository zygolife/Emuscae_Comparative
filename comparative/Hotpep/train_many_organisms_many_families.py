...
try:
	call(['rm -f', organism_array[0]+'/Results/output.txt'])
except:
	pass
for protein_dir_name in organism_array:
	...
	for cazy_class in cazyme_array:
		...
		call("python3 parallel_group_many_proteins_many_patterns_noDNA.py "+" ".join(str(x) for x in variables), shell=True)
	...
	if list_multidomain_enzymes == "yes":
		call("python3 list_multidomain_proteins.py "+protein_dir_name+" "+"_".join(cazyme_array), shell=True)
print("\nScreened\n"+"\n".join(organism_array))
